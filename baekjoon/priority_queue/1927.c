...
{
    while ((j != 1) && heap[j / 2] > x) // 입력값이 자식 노드보다 작으면
    {
...
void delete(int *heap, int j)
{
    ...
    while (child <= j)
    {
        if ((child < j) && heap[child] > heap[child + 1]) // 왼쪽 및 오른쪽 자식 노드를 비교 하여 더 작은 자식 노드 구하기
            child++;
        if (tmp <= heap[child]) // 마지막 값과 동일 숫자 => 부모를 제외하고 현재 자식 노드부터 끝까지 모두 크거나 같은 숫자 
            break ;
